...
namespace Window
{
    ...
    void MyMainWindow::InitRecentConnections() const
    {
    ...
    void MyMainWindow::InitMenu()
    {
        ...
        const auto actionConnect = menu->addAction("连接");
        menu->addSeparator();
        const auto avtionDelete = menu->addAction("删除");
        ...
        if (widget)
            ...
            connect(dynamic_cast<Window::FtpWidget *>(widget), &Window::FtpWidget::LoginOKSignal,
                    this, [=](Core::FtpUserData data)
                    {
                        ...
                        {
                            qDebug() << __FILE__ << __FUNCTION__ << " json has not key of root";
                            rootJson["root"] = nlohmann::json::array();
                        }
                        if (rootJson["root"].empty())
                        {
